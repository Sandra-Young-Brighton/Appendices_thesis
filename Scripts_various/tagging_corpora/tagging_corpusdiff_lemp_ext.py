...
class MWUTagger(object):
    ...
    def check_for_name(self, name):
        ...
        match name."""
        mwutagger = MWUTagger('JEFF_extendedmwutaggedALL_TI.vert', 'jeff_extendedmwutag_salmon.vert', 2, 'VTO_choice.txt')
        # check if tagged
        ...
        for name in mwutagger.names:
            ...
            try:
                ...
                if name == candidate:
                    ...
                    for i in range(n):
                        if '_ext-n' in self.buffer[i][-2]:
                            continue
                        else:
                            self.buffer[i][-2] = self.buffer[i][-2].replace('-n', '_ext-n') 
                            i += 1 
...
def main():
    mwutagger = MWUTagger('JEFF_extendedmwutaggedALL_TI.vert', 'jeff_extendedmwutag_div.vert', 2, 'VTO_choice.txt')
