...
def df_alt(relations):
    ...
    df_convert = relations.loc[relations['gramrel'].isin(['“X" is part of...', 'modifiers of X', 'nouns modified by X', 'X has part...', 'X is a ...', 'X is a type of...', 'X is the generic of...', "X's ..."])].copy()
       #'X %(3.lemma) .../... %(3.lemma) X', 'X and/or ...' (add later if wish)
...
def sort_table(path):
    ...
    #drop duplicates
    all_PC_rel = all_PC_rel.drop_duplicates(['source', 'target', 'hits', 'score'])
        
    #group and provide totals and means for hits and score (don't want to as just want to make sure there are no duplicates from inverted relations)
   #aggregated = all_PC_rel.groupby(['source', 'target'], as_index=False).aggregate({'hits': np.sum, 'score': [np.mean, np.median]})
    ...
    with open(path + '/aggregated_rel_'+anal+'_nodup.csv', 'w') as outfile: 
        all_PC_rel.to_csv(outfile, index=False)
