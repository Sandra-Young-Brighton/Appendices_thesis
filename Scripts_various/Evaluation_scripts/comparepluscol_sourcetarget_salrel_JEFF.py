...
def compare(jefffreq_file, input_path, jeffsal_file, output_path):
    #read in both dataframes
    df_jefffreq = pd.read_csv(jefffreq_file)
    df_jeffsal = pd.read_csv(input_path+jeffsal_file)
    ...
    no_path = os.path.basename(jeffsal_file)
    ...
    jeffsal_all_jefffreq_common.to_csv(output_path+'salall_freqmatch_'+f+'.csv', index=False)
    ...
    jefffreq_all_jeffsal_common.to_csv(output_path+'freqall_salmatch'+f+'.csv', index=False)
    ...
    #relations_x',
    jefffreq_only.to_csv(output_path+'jefffreq_only_'+f+'.csv', index=False)
...
#                     
def main():
    input_path = './JEFF_freqsalcomp/'
    output_path = './JEFF_freqsalcomp/output/'
    ...
    for jeffsal_file in filelist:
        if jeffsal_file.endswith('csv'):         
            list = compare('vto_comp_jeff_SCI_only_salfreq191202_rel_over5.csv', input_path, jeffsal_file, output_path)
            #printing to a file with the percentages
            df = df.append(pd.DataFrame([list], columns=('filter', 'JEFFfreqsal_total', 'JEFFrel5_total', 'JEFFfreqsal_not_in_JEFF_freq', 'JEFFfreq_not_in_JEFF_freqsal', 'JEFFfreqsal_in_JEFFfreq', '%_only_in_JEFFfreqsal', '%_only_in_JEFFfreq', '%_common_vs_JEFFfresal','%_common_JEFFfreq')))
    
    df.to_csv(output_path + 'comparison_percentages_jeff5vssal.csv', index=False)
