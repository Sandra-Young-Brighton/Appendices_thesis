...
def compare(corp1_file, input_path, corp2_file, output_path):
    ...
    df_jeff_ext_match = df_jeff_ext_match.drop_duplicates()
...
#                     
def main():
    input_path = './input_JEFF_EXT_scionly/'
    output_path = './output_JEFF_vs_EXT/'
    
    filelist = os.listdir(input_path)
    i = 1
    df = pd.DataFrame()
    jeff_file = input_path + 'jeff_SCI_only1704_rel_over0.csv'
	ext_file = input_path + 'JEFF_extendedonly_SCI_rel_over0.csv'
	
    # read in files as dataframe and change to lists
    list = compare(ext_file, input_path, jeff_file, output_path)
    ...
    df.to_csv(output_path + 'comparison_percentages_JEFFvsEXT.csv', index=False)
