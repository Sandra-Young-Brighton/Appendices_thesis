...
def sort_table(path):
    ...
    with open(path + '/aggregated_rel_WS_all_nodup.csv', 'w') as outfile: 
    ...
    totals_file = open(path + '/aggregated_rel_WS_all_nodup.txt', 'w')
...
def weighting(path):
    ...
    aggregated = pd.read_csv(path + '/aggregated_rel_WS_all_nodup.csv')
    ...
    while i <= 100:
        ...
        aggregated.to_csv(path + 'weighted/weighted_out/WS_all_rel_over'+str(i)+'.csv', index=False)
        i += 10
